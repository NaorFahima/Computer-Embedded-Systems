...
void main() {
    ...
    int current_switch = NONE; // current switch on, none at start
    int current_mode = NONE; // current mode currently running
    sys_config();

    while (1) {
        current_switch = current_switch_state();
        if (switches[6] && !switches[5]) {
            mode_sound();
            lcd_set_text(MODE_6, "   Beep Sound   ");
        }
        // case #3, #4, #6 are handled at mode function
        switch (current_switch) {
            case 0: // SW0 is on, do counter mode
                mode_counter(&current_mode);
                break;
            case 1: // SW1 is on, do shift mode
                mode_shift(&current_mode);
                break;
            case 2: // SW2 is on, do fan mode
                mode_fan(&current_mode, &fan_stage);
                break;
            case 5: // SW5 is on - skip
                lcd_set_text(MODE_5, "      Halt      ");
                break;
            case 7: // SW7 is on - exit the loop
                lcd_set_text(MODE_7, "      Exit      ");
                PORTA = 0x0;
                return;
            default: // no switches are on, clear screen
                if (!switches[6] && current_mode != NONE)
                    lcd_set_text("Flip The Switches!", "****************");
                break;
        }
        hold_program();
    }
...
void sys_config() {
    ...
    ANSELBbits.ANSB15 = 0;
    lcd_init(control, 7);
    lcd_set_text("Welcome to Lab2:", "Made by Gal&Naor");
